...
public class stage_2 extends BaseGameActivity implements
	...
	private int number_of_blocks = 7;
	...
	public void onLoadResources() {
		...
		mFailTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createFromAsset(this.mBitmapTextureAtlas, this, "fail.png", 0,
						256);
		// explosion
		explosionBitmapTextureAtlas = new BitmapTextureAtlas(512, 512, TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		mexplosionTextureRegion = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(this.explosionBitmapTextureAtlas, this, "explosion_new.png", 0, 0, 5, 5);
		...
		//**
		mFaceTextureRegion_box = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.mBitmapTextureAtlas, this, "bomb.png", 0, 0);
	...
	//	ball = new AnimatedSprite(centerX - this.mFaceTextureRegion.getHeight(), 410  , this.mFaceTextureRegion);
	...
	//	mMainScene.attachChild(ball);
		...
		/* Funny explosion. */
		explosion = new AnimatedSprite(centerX , 410, this.mexplosionTextureRegion);
		...
		final Sprite face = new Sprite(200, (CAMERA_HEIGHT+10) / 4, this.mFaceTextureRegion_box);
	...
	private void bomb_target() {
		...
		for(int i = 0; i < number_of_bombtarget; i++) {
			...
			AnimatedSprite bomb_target = new AnimatedSprite(x, y  , this.mFaceTextureRegion.deepCopy());
		...
		public void onUpdate(float pSecondsElapsed) {
			...
			while (targets.hasNext()) {
				...
				if(bomb.collidesWith(_target)){
					...
					//bomb.registerEntityModifier(new MoveModifier(1f, bomb.getX(), bomb.getX(),bomb.getY(),  bomb.getY(), EaseBounceOut.getInstance()) );
					//bomb.clearUpdateHandlers();
				
					
					
					
					
					// check for which dir the sprite collide ===
				/*	if(	cur_bomb_x < _target.getScaleCenterX())
							{ x_dir = _target.getScaleX() - new_width-1; }
					
					else if(cur_bomb_x > _target.getScaleCenterX() )
							{ x_dir = _target.getX() + _target.getWidthScaled()+1; }
					
					else if(cur_bomb_y < _target.getScaleCenterY())
							{ y_dir = _target.getScaleY() - _target.getHeightScaled()-1; }
				...
				*/	
					...
					switch(dir){
					case 1:
						x_dir = _target.getX() - bomb.getWidthScaled() -1; break;
					case 2:
						x_dir = _target.getX() + _target.getWidthScaled() + 1;  break;
					case 3:
						y_dir = _target.getY() - _target.getHeightScaled()-1; break;
					case 4:
						y_dir = _target.getY() + _target.getHeightScaled()+1; break;
					}
				...
				// bomb.setPosition(_target.getX() - new_width-1, bomb.getY());
				 move = true;
			...
			if(bomb.getWidth()+ bomb.getX() > CAMERA_WIDTH || 
					bomb.getX() < 0 ||
			...
			while (bomb_target_link.hasNext()) {
			 ...
			 explosion.setVisible(true);
				...
				while (projectiles.hasNext()) {
					...
					//_proj.registerEntityModifier(new MoveModifier(1f, _proj.getX(), _proj.getX(),_proj.getY(),  CAMERA_HEIGHT - _proj.getHeight(), EaseBounceOut.getInstance()) );
		 	...
		 	if(explosion.getCurrentTileIndex() == 24 && explosion.isVisible()){
...
//
		...
		if(pSceneTouchEvent.getAction()==TouchEvent.ACTION_DOWN){
	            firstX = pSceneTouchEvent.getX();  // remember the x, on the first touch
	            firstY = pSceneTouchEvent.getY();  // remember the x, on the first touch
	        } 
		
		
		
	        shootProjectile(firstX, firstY);
	...
	public void addTarget() {
		...
		for(int i = 0; i < number_of_blocks; i++) {
			...
			case 1:
				x = CAMERA_WIDTH /2  - bomb.getWidthScaled() - 5 ;
				y = CAMERA_HEIGHT - 200;
				break;
	...
	public void win() {
		if (mEngine.isRunning()) {
			...
			mEngine.stop(); 
			finish();
			startActivity(new Intent(this, stage_1.class));
